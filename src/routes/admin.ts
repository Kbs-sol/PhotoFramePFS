...
// ─── Hybrid brute-force rate limiter ─────────────────────────────────────────
// SECURITY FIX #12d: In-memory Map was used for login rate limiting, but Cloudflare
//   Workers can have multiple isolate instances — the Map resets on every cold start
//   and is NOT shared between instances, making it useless in production.
//
//   Solution: Two-layer approach:
//   1. Module-level Map: catches burst attacks within a single isolate lifetime (fast path)
//   2. DB-backed counter: persists across cold starts and all instances (durable path)
//      Writes to `login_attempts` table: { ip, count, reset_at }
//
//   The in-memory layer prevents DB hammering on repeat attempts from the same isolate.
//   The DB layer ensures coordinated blocking across the full CF edge network.

const _inMemoryAttempts = new Map<string, { count: number; resetAt: number }>();

function checkInMemoryRateLimit(ip: string): boolean {
  const now = Date.now();
  const record = _inMemoryAttempts.get(ip);
  if (!record || now > record.resetAt) {
    _inMemoryAttempts.set(ip, { count: 1, resetAt: now + 15 * 60 * 1000 });
    return true;
  }
  if (record.count >= 5) return false; // fast-path block (5 attempts in-memory)
  record.count++;
  return true;
}

async function checkDbRateLimit(env: any, ip: string): Promise<boolean> {
  if (!env?.SUPABASE_URL) return true; // No DB — fall back to in-memory only
  try {
    ...
    if ((count || 0) >= 10) return false; // DB-backed block
    ...
    // DB error — degrade gracefully (don't block on DB failure)
    return true;
  }
}
...
admin.post('/auth', async (c) => {
  try {
    ...
    // 1b. If bypass fails (wrong email/password) AND Supabase is not configured → reject gracefully
    // Without this guard, calling getSupabase(env) with undefined SUPABASE_URL throws a hard crash
    if (!c.env.SUPABASE_URL) {
      return c.json({ 
        error: 'Authentication failed.',
        hint: isOwnerEmail 
          ? 'Password mismatch. Verify ADMIN_PASSWORD env var.' 
          : 'Only the owner email can log in without Supabase configured.',
        demo_info: {
          note: 'Demo / No-Supabase mode: set env vars OWNER_EMAIL + ADMIN_PASSWORD + ADMIN_TOKEN to enable login',
          placeholder_email: 'admin@chitraframe.in',
          placeholder_password: 'ChitraFrame@2024',
          setup_guide: 'Go to Cloudflare Dashboard → Workers → Settings → Variables to configure real credentials'
        }
      }, 401);
    }
    ...
    // NOTE: select 'id,role' — role column added via schema.sql patch;
    //       if column doesn't exist yet, fall back to 'admin'
...
admin.use('/*', async (c, next) => {
  ...
  if (c.env.ADMIN_TOKEN && token === c.env.ADMIN_TOKEN) {
    (c as any).set('user', { email: c.env.OWNER_EMAIL || c.env.ALERT_EMAIL || 'admin@chitraframe.in', role: 'superadmin' });
    return next();
  }

  // If ADMIN_TOKEN bypass failed AND Supabase is not configured → reject without crashing
  if (!c.env.SUPABASE_URL) {
    return c.json({ error: 'Unauthorized: Supabase not configured and token invalid.' }, 401);
  }
  ...
  (c as any).set('user', { ...user, role: (adminUser as any).role || 'admin' });
  return next();
});
...
admin.put('/products/:id', async (c) => {
  ...
  // Strip columns that don't exist on the products table (defence-in-depth)
  delete body.base_price;  // pricing lives in product_variants
  delete body.id;          // never mutate the PK
...
admin.post('/logistics/generate-label', async (c) => {
  ...
  if (labels.length > 0) {
    // shiprocket_label_url column added via schema.sql patch
    // Update gracefully — if column doesn't exist yet, error is silently ignored
    const updatePayload: any = { updated_at: new Date().toISOString() };
    try { updatePayload.shiprocket_label_url = labels.join(','); } catch {}
    await sb.from('orders').update(updatePayload).eq('order_id', orderId);
...
admin.post('/reviews/:id/approve', async (c) => {
  const id = c.req.param('id');
  const sb = getSupabase(c.env);
  const { data } = await sb.from('reviews').update({ is_approved: true, is_hidden: false }).eq('id', id).select().single();
...
admin.post('/reviews/:id/hide', async (c) => {
  const id = c.req.param('id');
  const sb = getSupabase(c.env);
  const { data } = await sb.from('reviews').update({ is_hidden: true, is_approved: false }).eq('id', id).select().single();
  return c.json({ success: true, review: data });
});
...
admin.post('/reviews/:id/feature', async (c) => {
  ...
  const { data } = await sb.from('reviews').update({ is_featured: !cur?.is_featured }).eq('id', id).select().single();
  return c.json({ success: true, review: data });
});
...
admin.post('/reviews/:id/reply', async (c) => {
  ...
  const sb = getSupabase(c.env);
  const { data } = await sb.from('reviews').update({ admin_reply: reply || null }).eq('id', id).select().single();
  return c.json({ success: true, review: data });
});
...
admin.post('/review-requests', async (c) => {
  ...
  await sb.from('review_requests').insert({
    order_id: orderId, email, name: name || null
  });
...
// ==========================================
// LOGISTICS — /orders/* ALIASES
// Mirrors what admin.js calls: /orders/:id/shiprocket, /orders/:id/awb,
// /orders/:id/pickup, /orders/bulk-sync (maps to /logistics/* endpoints)
// ==========================================

// POST /orders/bulk-sync  (alias for /logistics/sync-pending)
admin.post('/orders/bulk-sync', async (c) => {
  if (!c.env.SUPABASE_URL) return c.json({ synced: 0, failed: 0, results: [] });
  const sb = getSupabase(c.env);
  const { data: orders } = await sb.from('orders')
    .select('*')
    .eq('shiprocket_synced', false)
  ...
  for (const order of orders || []) {
    const result = await createShiprocketOrder(c.env, order);
    if (result.success && (result as any).shiprocketOrderIds) {
      await sb.from('orders').update({
        shiprocket_synced: true,
        shiprocket_order_id: (result as any).shiprocketOrderIds.join(','),
        updated_at: new Date().toISOString()
      }).eq('order_id', order.order_id);
    }
    results.push({ orderId: order.order_id, ...result });
  }
  return c.json({ synced: results.filter((r: any) => r.success).length, failed: results.filter((r: any) => !r.success).length, results });
});

// POST /orders/:orderId/shiprocket  (alias for /logistics/create-shiprocket-order)
admin.post('/orders/:orderId/shiprocket', async (c) => {
  const orderId = c.req.param('orderId');
  const sb = getSupabase(c.env);
  const { data: order } = await sb.from('orders').select('*').eq('order_id', orderId).single();
  if (!order) return c.json({ error: 'Order not found' }, 404);
...
// POST /orders/:orderId/awb  (alias for /logistics/generate-awb)
admin.post('/orders/:orderId/awb', async (c) => {
  const orderId = c.req.param('orderId');
  const sb = getSupabase(c.env);
  const { data: order } = await sb.from('orders').select('shiprocket_order_id').eq('order_id', orderId).single();
  if (!order?.shiprocket_order_id) return c.json({ error: 'Shiprocket order not created yet' }, 400);
  const srIds = order.shiprocket_order_id.split(',');
  const result = await generateAWB(c.env, srIds);
  if (result.success && (result as any).awbNumbers) {
    await sb.from('orders').update({
      awb_number: (result as any).awbNumbers.join(','),
      updated_at: new Date().toISOString()
    }).eq('order_id', orderId);
  }
  return c.json(result);
...
// POST /orders/:orderId/pickup  (alias for /logistics/schedule-pickup)
admin.post('/orders/:orderId/pickup', async (c) => {
  const orderId = c.req.param('orderId');
  const sb = getSupabase(c.env);
  const { data: order } = await sb.from('orders').select('shiprocket_order_id').eq('order_id', orderId).single();
  if (!order?.shiprocket_order_id) return c.json({ error: 'Shiprocket order not created' }, 400);
  const srIds = order.shiprocket_order_id.split(',');
  const result = await schedulePickup(c.env, srIds);
  if (result.success) {
    await sb.from('orders').update({
      pickup_status: 'scheduled',
      updated_at: new Date().toISOString()
