...
export function ownerNewOrderAlert(order: any, whatsappUrl: string): string {
  ...
  const itemsRows = (order.items || []).map((item: any) => {
    ...
    const downloadBtn = hasImage && !item.image_url.startsWith('data:')
      ? `<a href="${item.image_url}" download style="background:#DAA520;color:#000;padding:4px 10px;border-radius:4px;font-size:11px;font-weight:700;text-decoration:none;">⬇ Download Image</a>`
  ...
  return `
    <div style="font-family:Arial,sans-serif;max-width:680px;margin:0 auto;background:#111;color:#eee;border-radius:12px;overflow:hidden;">
      <!-- Header -->
      <div style="background:#DAA520;padding:20px 28px;display:flex;justify-content:space-between;align-items:center;">
        <div>
          <div style="font-size:20px;font-weight:800;color:#000;">🖼️ PhotoFrameIn</div>
      ...
      <div style="padding:24px 28px;">
        ...
        <table style="width:100%;border-collapse:collapse;margin-bottom:20px;">
          ...
          <tr>
            ...
            <td style="padding:6px 0;font-size:14px;"><a href="tel:${order.customer_phone}" style="color:#DAA520;">${order.customer_phone}</a></td>
          </tr>
          <tr>
            <td style="padding:6px 0;color:#aaa;font-size:12px;">Email</td>
            <td style="padding:6px 0;font-size:13px;"><a href="mailto:${order.customer_email}" style="color:#DAA520;">${order.customer_email}</a></td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td style="padding:6px 0;font-size:18px;font-weight:800;color:#DAA520;">₹${order.total?.toLocaleString('en-IN')}</td>
          </tr>
        </table>
        ...
        <div style="margin-top:24px;display:flex;gap:12px;flex-wrap:wrap;">
          ...
          <a href="https://photoframepfs.pages.dev/admin" style="background:#DAA520;color:#000;padding:10px 20px;border-radius:8px;text-decoration:none;font-weight:700;font-size:14px;">🔧 Open Admin Panel</a>
        </div>
